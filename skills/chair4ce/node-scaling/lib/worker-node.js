...
const LARGE_PROMPT_TIMEOUT_MS = 90000; // Extended timeout for large prompts
const LARGE_PROMPT_THRESHOLD = 10000;  // Characters — prompts above this get extended timeout
...
class WorkerNode {
  ...
  async execute(task) {
    ...
    // Auto-extend timeout for large prompts (synthesis, multi-phase aggregation)
    const promptLength = (task.instruction || '').length + (task.context || '').length + (typeof task.input === 'string' ? task.input.length : 0);
    const autoTimeout = promptLength > LARGE_PROMPT_THRESHOLD ? LARGE_PROMPT_TIMEOUT_MS : DEFAULT_TASK_TIMEOUT_MS;
    const timeoutMs = task.timeoutMs ?? config.scaling?.timeoutMs ?? autoTimeout;
    let lastError = null;
...
Provide a focused, high-quality response. Be concise — prioritize insight density over length.`;
    ...
    // Auto-increase output tokens for large prompts (synthesis tasks need room)
    const totalInputLength = prompt.length;
    if (totalInputLength > LARGE_PROMPT_THRESHOLD && !task.maxOutputTokens) {
      llmOptions.maxTokens = 8192;
    }
  ...
  validateOutput(result, task) {
    ...
    // Only fail for short-to-medium responses; long responses (>2000 chars) that are 
    // truncated still have value and shouldn't be discarded entirely
    const lastChar = response.trim().slice(-1);
    const hasTerminal = /[.!?\n\]\)`"'*\-\d]/.test(lastChar);
    if (!hasTerminal && response.length > 500 && response.length < 2000) {
